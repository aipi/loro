...
fn scan_pii(text: &str) -> Vec<(String, String)> {
    ...
    for w in text.split_whitespace() {
        ...
        // NOT a known tail — a domain (`exemplo.com`) and a file (`context.md`) have
...
";
    ...
    fn the_pii_scan_finds_a_person_and_leaves_a_role_alone() {
        ...
        assert!(achados
            .iter()
            .any(|(k, v)| k == "handle" && v == "carlos.pereira"));

        // uma caixa funcional é papel, não pessoa
        let papel = scan_pii("escreva para sinistros@exemplo.com.br");
