...
mod config;
mod proc;
// Meeting mode’s system-audio track (ADR-0031): macOS spawns the Swift/
...
pub(crate) struct AppState {
    ...
    // the Windows counterpart is a thread, not a process (ADR-0031)
    #[cfg(windows)]
...
// The in-process capture path (ADR-0031). `Some` means this platform captures
...
pub(crate) fn system_capture_start(app: &AppHandle, state: &AppState) -> Result<String, String> {
    // Windows captures in-process (ADR-0031). Returning through a shim instead
