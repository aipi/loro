...
- **Capture & Transcription** — audio source selection (mic / system / meeting),
  ...
  **system** resolve to a single `whisper-stream` capture index (`-c`): the system
  ...
  `whisper-cli` (reusing file mode). The capturer is per platform (ADR-0033):
...
| `interpreter_devices` | — | `[{id,name}]` / err | ADR-0035: output devices the interpreter can speak through, as `say -a '?'` enumerates them. The `id` is valid **only for this moment** — the caller stores the `name` (a `coreaudiod` restart renumbers every device, measured 2026-09-06). macOS-only; elsewhere `err.interpreter_platform_unsupported` |
| `interpreter_voices` | — | `[{name,locale}]` / err | ADR-0035: the installed **English** voices (`say -v '?'`, filtered to `en_*` because `-tr` produces nothing else). Novelty voices are *not* filtered out — the UI previews instead, per the ADR-0034 lesson on blocklists |
| `interpreter_translate` | `data, model, lang` | `{text}` / err | ADR-0035: one utterance (webm bytes, already cut on silence by the frontend) → English text, via `whisper-cli -tr`. Refuses a non-translating model up front (`err.model_cannot_translate:<id>` — `large-v3-turbo` ignores `-tr` silently). Runs off the main thread |
| `interpreter_speak` | `text, voice, device` | `()` / err | ADR-0035: speak the English text through `device` (**by name**; the id is resolved at call time). Text goes to `say` by file, never as an argument — a translation starting with `-` would parse as a flag, and speech content stays off any command line (BR-8) |
| `interpreter_audio_setup` | — | `{state, command}` / err | ADR-0036 §6.5: which of three states the virtual driver is in — `ready`, `installed_not_loaded` (measured: the driver sits in `/Library/Audio/Plug-Ins/HAL` and `coreaudiod` only scans it at start, so *installed* and *available* are different states), or `missing`. Returns the command for the current step and **runs nothing** — install and `killall coreaudiod` need admin, and an app that runs `sudo` out of sight is what nobody should build |
| `interpreter_neural_status` | — | `{binary, model, missing[], sample}` / err | ADR-0036: whether the cloned voice can run. Binary, model and voice sample go missing **separately**, so the screen names which one instead of a mute "unavailable" |
| `voice_sample_status` | — | `{phrases[], totalMs, enough, minTotalMs}` / err | ADR-0036 §3: the user's voice sample. `phrases` carries the **fixed text to read** — cloning needs the audio *with its transcript*, and a free recording would make that transcript a guess. `enough` is derived, never stored, so a file deleted by hand shows on screen at once |
| `voice_sample_save` | `index, data` | `SampleStatus` / err | one phrase: webm in, 16 kHz mono WAV on disk (`~/.loro/voice-sample/`, user data — never the acervo, never a PR, never off the machine). Refuses a reading under 1.2s (`err.voice_sample_too_short:<ms>`) *before* replacing a good one. Runs off the main thread |
| `voice_sample_clear` | — | `SampleStatus` / err | delete the recording. First-class because it is the person's own voice: whoever recorded it must be able to remove it without hunting for a folder |
| `voice_install_status` | — | `{parts[], remainingBytes, ready, supported}` / err | ADR-0036 §6.5: the three pieces of the cloned-voice engine, each with its pinned URL, size and SHA-256. `remainingBytes` is what is still to download — ~180 MB is not something to fetch without saying so. `supported: false` off macOS/Linux/Windows-x64, and the UI then offers nothing |
| `voice_install_part` | `part` | `InstallStatus` / err | download one piece over HTTPS (system `curl`, `--proto =https`), **verify its pinned SHA-256 before extracting**, then install atomically; emits `voice-install-progress { part, downloaded, total }`. The engine lands as `bin/` + `lib/` **siblings** — that is what `@loader_path/../lib` needs, and `~/.loro/bin` is already in `known_bin_dirs()` so the app finds what it installs (ADR-0030). Idempotent |
| `read_aloud_voices` | — | `[{name,locale}]` / err | ADR-0037: the system voices, **all locales** — unlike the interpreter's list, which is `en_*` only because `-tr` makes English and nothing else. Here the text is the document's, normally Portuguese |
| `read_aloud_start` | `text, voice?, lang?` | `{speaking,paused,supported}` / err | speak the text on the **default** output device (the person's headphones) — no `-a`, unlike the interpreter which forces a chosen device. With no `voice`, picks the first *preferred* voice for `lang` (measured: `say` lists pt_BR as Eddy·Flo·Grandma·Grandpa·Luciana, so "first of locale" chose a character voice). Kills any reading in flight and stores the new one **under one lock** — two separate locks left a window where two voices overlapped |
| `read_aloud_pause` | — | `ReadState` / err | toggle pause by signal (`SIGSTOP`/`SIGCONT` via `/bin/kill`), never kill-and-restart: restarting would return to the top of the document. Measured 2026-09-08 — SIGSTOP gives **−91 dB** (digital silence) within the second, SIGCONT resumes where it stopped |
| `read_aloud_stop` | — | `ReadState` / err | end the reading and sweep its temp files. Also called when the window hides and on app exit: closing the Loro window only *hides* it, and a `say` child outlives its parent |
| `read_aloud_state` | — | `ReadState` | derived from the process (`try_wait`), never a stored flag — a reading that ended on its own must read as stopped, else the button offers to pause what already went quiet. `supported: false` off macOS so the screen can tell "not reading" from "not available here" |
...
| `update_open_release` | — | `()` / err | open the release page in the system browser. The app never downloads or installs the update (ADR-0032) |
...
| `voice-install-progress` | `{part, downloaded, total}` | one cloned-voice piece downloading, in bytes (ADR-0036). Paints the **same** bar as `model-download-progress`: the pieces are offered as rows of the model manager, so the screen has one way to draw "something large to download" |
...
| Meeting capture | mic + system audio, mixed late — ScreenCaptureKit sidecar (macOS), in-process WASAPI loopback (Windows) | ADR-0001 §2, ADR-0033 |
| Interpreter mode | your speech out as an English voice through a chosen device; consecutive (cut on silence), off by default, driver **guided** not packaged | ADR-0035 |
