...
| `brain_move_meeting` | `rel, destSlug` | new rel | move a whole meeting folder into another brainstorming's `reunioes/`, rewriting the `tema` it records in `manifest.json` and in `reuniao.md`'s front matter; never overwrites — ADR-0017 |
