...
Reported by the owner after using the destination against `acervo-remoto/acervo` (a
...
review taking long to load. Measured on `acervo-remoto/acervo`, one click on the
destination:
...
editando o rascunho…"). Seen in the running app against `acervo-remoto/acervo` #6. It
...
### Found by driving the app against the real remote (`acervo-remoto/acervo`)
