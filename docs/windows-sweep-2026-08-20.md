...
| Meeting | **was BY DESIGN, now WORKS** | On the sweep date the gate was correct: `hostOs !== "macos"` → toast `err.meeting_macos_only`, and the selector dropped the option. **Superseded by ADR-0033**: Windows captures system audio through WASAPI loopback, so the mode is offered here and the error code is now `err.meeting_unsupported_os`, which Linux still gets. This row is kept rather than rewritten — the document is a dated snapshot, and what it recorded was true |
